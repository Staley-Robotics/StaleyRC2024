...
class NTTunableFloatArray:
    """
    NTTunableFloatArry is a custom class designed to:
        1. Publish an array of Float values to NetworkTables (with persistence if requested)
        2. Store the Float array in memory for faster access
    ...
    def __init__( self,
        ...
        self.value = float( self.ntTbl.getNumberArray( self.name, value ) )
        if not self.ntTbl.putNumberArray( self.name, self.value ):
            raise Exception(
    ...
    def set(self, value:float) -> None:
        ...
        """
        if self.ntTbl.putNumberArray( self.name, value ):
            self.value = value
        else:
            raise Exception(
                f"{self.__class__.__str__()}: Network Table Value already exists as different type."
